...
## Installation 
Assumed that you have already installed `rapidjson`, details of installation of which could be viewed on the corresponding hyperlink. But generally, you can install just by using package manager:
...
```
 ...
 Download the `mjson-cpp` repository from GitHub:
...
auto matrix3d = pm.get<std::vector<std::vector<std::vector<std::complex<double>>>>>(keypath);
